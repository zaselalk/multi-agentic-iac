...
PROMPT_ENHANCEMENT_STRATS = ["RAG", "COT", "FSP", "multi-turn", "multi-agent", ""]  # added multi-agent
...
Goal: Convert the intent into deployable, policy-compliant Terraform HCL. Think internally; DO NOT reveal your steps. Output ONLY the final HCL.

Retrieved context:
{knowledge if knowledge.strip() else "None"}

Internal process (do NOT output these steps):
1) Requirements → extract exact names/strings and constraints from the task and context.
2) Architecture → select the minimal AWS resources required; avoid external modules/data sources unless strictly necessary.
3) IR Synthesis → assemble a typed internal IR (JSON in your head) with providers, resources, attributes, references, dependencies.
4) HCL Generation → produce valid Terraform HCL from the IR.
5) Verifier (auto-fix loop) → silently validate:
   - HCL is syntactically valid; no placeholders; no undefined refs; identifiers use underscores.
   - Use only attributes that exist in the latest AWS provider schema (e.g., replace deprecated bucket `acl` with `aws_s3_bucket_acl`, use `source = data.archive_file.*.output_path` instead of unsupported `output_base64`).
   - `terraform` block pins versions; provider and `var.region` exist (default "us-east-1").
   - All taggable resources have `tags = local.default_tags` (Owner, Environment, CostCenter).
   - S3 (if present) blocks public access and has SSE; no public ACLs/grants.
   - Security groups avoid 0.0.0.0/0 unless explicitly requested; ports are minimal.
   - EBS/RDS encryption on (`encrypted = true` / `storage_encrypted = true`).
   - Names/strings match the task EXACTLY (case-sensitive) with no prefixes/suffixes.
   - Prefer `jsonencode({{...}})` for IAM/policy docs.
   - Implicit refs or `depends_on` ensure correct plan order.
   If any check fails, fix and re-verify before emitting. Repeat until all pass.

Compliance constraints (must hold):
• Terraform & provider
  - Terraform >= 1.5; AWS provider ~> 5.0; define `provider "aws"` with `region = var.region` and `variable "region"` default "us-east-1".
  - No undeclared variables; avoid external modules/data unless required.

• Determinism & names
  - Match task strings EXACTLY. Avoid non-determinism (`random_*`) unless the task explicitly asks for it; if it does, use only the required `random_*` resources.

• Tags (mandatory)
  - Include:
    locals {{ default_tags = {{ Owner = "iac-eval", Environment = "dev", CostCenter = "1234" }} }}
  - Apply `tags = local.default_tags` to every taggable resource.

• Security & compliance defaults
  - S3: block public access; enable SSE (AES256 or KMS); no public ACLs/grants.
  - Security groups: no `0.0.0.0/0` unless the task demands it.
  - EBS/RDS: encryption enabled.

• HCL validity & structure
  - Valid HCL only; no stray backticks; no trailing commas; use underscores in identifiers; hyphens only inside string values.
  - Use implicit references / `depends_on` as needed.
  - Avoid live lookups; prefer attributes from resources you define.

• OPA policies (v0.65)
  - Satisfy typical org policies: required tags, encryption at rest, blocked public S3, minimal network exposure, provider/version pinning, exact names/regions.

EXAMPLE (for guidance only; do NOT copy into your output):
Example prompt: Create an AWS RDS instance (instance_class db.t2.micro) and do not create a final snapshot before deletion; generate a random identifier suffix and a random password.
Example output:
```hcl
terraform {{
...
- Begin the response with EXACTLY: ```hcl
- End the response with EXACTLY: ```
- Produce ONE fenced code block only (no prose before or after).
- If you need to explain, put brief # comments INSIDE the HCL block only.

Now write the Terraform HCL for this task.
{user_prompt}
""".strip()
...
def separate_answer_and_code(text, delimiters=["```hcl"]):
    for delimiter in delimiters:
        # split once at the language fence start
        parts = text.split(delimiter, 1)
        if len(parts) < 2:
            # delimiter not found, continue trying others
            continue
        # the first part is the answer
        answer = parts[0].strip()
        # take content only up to the FIRST closing fence to avoid including Evidence/JSON blocks
        rest = parts[1]
        end_idx = rest.find("```")
        code = rest[:end_idx].strip() if end_idx != -1 else rest.strip()
        if code != "":
            return answer, code
    # fallback: no fences found
    return text.strip(), ""
...
def prompt_enhancements(prompt, PROMPT_ENHANCEMENT_STRAT, Retriever):
    ...
    elif PROMPT_ENHANCEMENT_STRAT == "multi-agent":
        knowledge = rag_knowledge(Retriever, prompt) if Retriever else ""
        prompt = build_multi_agent_prompt(knowledge, prompt)
    else:
...
def model_evaluation(
    ...
    for i in range(NUM_EXISTING_SAMPLES, NUM_SAMPLES_PER_TASK):
        ...
        while True:
            ...
            df.at[index, "LLM OPA match phase Error #" + str(i)] = x["opa_evaluation_error"]
            ...
            # After pipeline run:
            if str(x["opa_evaluation_result"]).strip().lower() == "success":  # fixed case
                num_correct += 1
                break
                ...
                # Validator-driven repair loop (max 4 attempts)
                if multi_turn_count == 4:
                    break
                multi_turn_count += 1
                if code == "":
                    repair_user_prompt = f"""Your previous output contained no Terraform code. Reattempt.
Original intent:
{row["Prompt"]}
Ensure the first fenced block is valid Terraform HCL."""
                else:
                    if not x["terraform_plan_success"]:
                        repair_user_prompt = prompt_templates.multi_turn_plan_error_prompt(
                            row["Prompt"], code, x["terraform_plan_error"]
                        )
                    elif x["opa_evaluation_result"] == "Failure":
                        repair_user_prompt = prompt_templates.multi_turn_rego_error_prompt(
                            row["Prompt"], code, policy_file, x["opa_evaluation_error"]
                        )
                    else:
                        break
                knowledge = rag_knowledge(Retriever, row["Prompt"]) if Retriever else ""
                prompt = build_multi_agent_prompt(knowledge, repair_user_prompt)
                preprompt = prompt_templates.multi_turn_system_prompt()
                continue
            else:
                break
...
def main(
    ...
    """
    Evaluate models.
    Available enhancement strategy: "RAG", "COT", "FSP", "multi-turn", or "multi-agent".
    ...
    if "gemini-1.0-pro" in models or "gemini-2.5-pro" in models or "gemini-2.0-flash" in models:
        set_google_credentials()

    if "gpt3.5" in models or "gpt4" in models or "gpt5" in models:
        setup_gpt_client()
